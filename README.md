...
- 双向/三端同步：只有一端变化时自动传播；多端分别修改时转为冲突，不按时间戳盲目覆盖。
...
- 未开启时：发现一端笔记已删除，其他端副本保持不动。
- 开启时：Joplin 副本进入 Joplin 废纸篓；Obsidian Markdown 文件进入 Windows 回收站。
- 第一版不会自动删除思源文档，因为目前没有在本项目中验证过可等同于系统回收站的安全删除接口。
...
2. 点击“测试所选连接”。
3. 点击“刷新所选端目录”。
4. 选择单向或双向同步。单向模式明确选择一个来源并勾选一个或两个目标；双向模式使用连接区启用的两端或三端。
...
- 一端删除、另一端又修改：标为冲突，不自动决定删除或保留。
- 缺失附件、Vault 外部附件或同名附件无法唯一确定：标为冲突，不覆盖目标。
- 重复同步 ID、同一路径多条笔记：必须先在原笔记软件中整理，软件不会猜测配对关系。

逐块合并只处理 Markdown 正文。标题、标签和目录由合并窗口中选定的元数据来源决定。合并方案只有在主窗口再次点击“执行”后才会真正写入。
